...
PANELBAKU_API_KEY = os.getenv("PANELBAKU_API_KEY", "aecefd5db90f0ca63ccc3d3d7683732e")
# Bu istifadəçi ID-lərinin sifarişləri PanelBaku-ya GÖNDƏRİLMİR - admin manual idarə edir.
...
# ===== PANELBAKU SMM API =====
async def panelbaku_get_balance():
    """PanelBaku hesabının cari balansını çəkir. (balance: float, currency: str) və ya (None, None) xəta halında."""
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.post(PANELBAKU_API_URL, data={
                'key': PANELBAKU_API_KEY,
                'action': 'balance'
            })
            data = resp.json()
        ...
        logger.error(f"panelbaku_get_balance error: {e}")
        return None, None

async def panelbaku_add_order(service_id, link, quantity):
    """
    PanelBaku-ya sifariş göndərir.
    Qaytarır: (success: bool, panel_order_id_or_None, error_message_or_None)
    success=False + error_message daxilində 'bakiye'/'balance' keçirsə, bu balans yetersizliyi deməkdir.
    """
    try:
        ...
        logger.error(f"panelbaku_add_order error: {e}")
        return False, None, str(e)

def is_insufficient_balance_error(error_message):
    """PanelBaku-nun balans yetersizliği xətasını tanıyır (panel müxtəlif dillərdə mətn qaytara bilər)."""
...
async def try_send_order_to_panel(order_id, user_id, service_id, link, quantity, context):
    """
    Bir sifarişi PanelBaku-ya göndərməyə cəhd edir və DB-ni + log kanalını uyğun şəkildə yeniləyir.
    ...
    """
    success, panel_order_id, error_msg = await panelbaku_add_order(service_id, link, quantity)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('awaiting_profile_link'):
        ...
        if not is_excluded_user and api_service_id:
            # quantity ürün adından çıkarılamıyor; admin /urunservis ile servis bağlarken
            # min/max miktarı PanelBaku'nun kendi servis ayarına göre otomatik kabul ediyoruz.
            # Burada quantity=1 sipariş başına ürün adedidir (örn. "1k Takipçi" tek seferde 1000 gönderilecekse
            # admin /urunservis ile miktarı da kaydedebilir; basitlik için ürünün price alanına dokunmuyoruz).
            quantity = product.get('api_quantity') or 1
            panel_result = await try_send_order_to_panel(
                order_id, user_id, api_service_id, profile_link, quantity, context
            )
        ...
        if panel_result == 'sent':
            log_text += f"\n\n✅ <b>PanelBaku'ya otomatik gönderildi.</b>"
            ...
            log_text += (
                ...
                f"PanelBaku bakiyeniz yetersiz olduğu için sipariş otomatik gönderilemedi. "
...
async def admin_set_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mevcut bir ürünü PanelBaku service ID'sine bağlar: /urunservis <ürün_id> <service_id> [miktar]
    Miktar verilmezse varsayılan 1 kullanılır (PanelBaku'ya gönderilecek adet)."""
    if not is_admin(update.message.from_user.id):
        await update.message.reply_text('❌ Yetkiniz yok!')
        return
    if len(context.args) < 2:
        await update.message.reply_text(
            '📝 Kullanım: /urunservis <ürün_id> <service_id> [miktar]\n\n'
            'Örnek: /urunservis 5 3868 1000\n'
            '(Bu, ürün ID 5\'i PanelBaku\'nun 3868 nolu servisine bağlar, her siparişte 1000 adet gönderir)'
        )
        return
    ...
    try:
        ...
        c.execute(
            'UPDATE products SET api_service_id = %s, api_quantity = %s WHERE product_id = %s',
            (service_id, quantity, product_id)
        )
        conn.commit()
        await reply_rich(
            update.message,
            ...
            f'📝 Ürün: {product["name"]}\n'
            f'🔗 PanelBaku Servis ID: {service_id}\n'
            f'🔢 Miktar: {quantity}\n\n'
            f'Bu üründen sipariş gelince otomatik olarak PanelBaku\'ya gönderilecek.'
        )
    except Exception as e:
...
async def admin_panel_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    balance, currency = await panelbaku_get_balance()
    if balance is None:
...
async def admin_products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        c = conn.cursor(cursor_factory=RealDictCursor)
        c.execute('SELECT product_id, name, category, price, stock, vip_only, api_service_id FROM products')
        products = c.fetchall()
    except Exception as e:
    ...
    for p in products:
        ...
        stock_text = 'Sınırsız' if p['stock'] >= 999999 else str(p['stock'])
        api_tag = f"🔗API:{p['api_service_id']}" if p.get('api_service_id') else ''
